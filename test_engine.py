...
def test_net_rpn(test_split, topn_rois=cfg.TOPN_ROIS, threshold=cfg.OVERLAP_THRESHOLD, topk=cfg.TOPK, vis=False):
    ...
    dp = DDPNDataProvider(data_split=test_split, batchsize=1)
...
def exec_validation(test_split, batchsize, gpu_id, prototxt, caffemodel, use_kld=cfg.USE_KLD, use_reg=cfg.USE_REG,
                    threshold=cfg.OVERLAP_THRESHOLD, topk=cfg.TOPK, vis=False):
    caffe.set_mode_gpu()
    caffe.set_device(gpu_id)
    net = caffe.Net(prototxt, caffemodel, caffe.TEST)
    return test_net(test_split, net, batchsize, use_kld=use_kld, use_reg=use_reg, threshold=threshold, topk=topk,
                    vis=vis)
